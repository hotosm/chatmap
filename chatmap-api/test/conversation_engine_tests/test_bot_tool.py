...
def _make_bot_tool(bot_state_store=None, message_to_send_store=None, survey_responses_store=None) -> BotTool:
    return BotTool(
...
async def test_call_decrypts_text_and_delegates_to_the_flow():
    ...
    bot_tool = _make_bot_tool(bot_state_store, message_to_send_store, survey_responses_store)
    fake_flow = AsyncMock()
    ...
    fake_flow.call.assert_awaited_once_with(
        ...
        context=BotFlowContext(
            ...
            message_id="msg-1", point_id=None, bot_state_store=bot_state_store,
        ),
    )
