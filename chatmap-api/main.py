...
async def qr(session: dict = Depends(get_current_session)):
    async with httpx.AsyncClient() as client:
        response = await client.get(f'{server_url}/start-qr?session={session}')
        if response.status_code != 200:
            logger.info(f'Failed to get QR code: {session}')
...
async def status(session: dict = Depends(get_current_session)) -> Dict[str, str]:
    async with httpx.AsyncClient() as client:
        response = await client.get(f'{server_url}/status?session={session}')
        if response.status_code != 200:
...
async def status(session: dict = Depends(get_current_session)) -> Dict[str, str]:
    async with httpx.AsyncClient() as client:
        response = await client.get(f'{server_url}/logout?session={session}')
        if response.status_code != 200:
            raise HTTPException(status_code=502, detail="Failed to logout")
        return {'status': "logged out"}

# Cleanup old messages
async def cleanup(session: dict = Depends(get_current_session)):
    cutoff_time_ms = int(time.time() * 1000) - EXPIRING_MIN_MS
    cutoff_id = f"{cutoff_time_ms}-0"
    entries = await redis_client.xrange(f"{STREAM_KEY}:{session}", min='-', max=cutoff_id)
    # Remove data older than (EXPIRING_MIN_MS) minutes
    for entry_id, _ in entries:
        await redis_client.xdel(f"{STREAM_KEY}:{session}", entry_id)
...
### FIXME: get user from session, so it works when there's no data in Redis
@app.get("/chatmap")
async def get_chatmap(session: dict = Depends(get_current_session), db: Session = Depends(get_db)) -> GeoJson:
    logger.info(f'get_chatmap: session {session}')
    try:
        # Get all available messages
        entries = await redis_client.xrange(f"{STREAM_KEY}:{session}", min='-', max='+')
        # Cleanup old messages
    ...
    if len(data) > -1:
        ...
        for property in geoJSON['features']:
            property['properties']['message'] = decrypt(property['properties']['message'])
