...
import {
    ...
    emptyQuestion, isQuestion, maxAttemptsFromSetup, maxAttemptsFromTemplate,
    maxAttemptsHasProblems, messagesFromSetup, messagesFromTemplate,
    messagesToSave, problemsIn, questionsOf,
...
export default function BotSetup() {
    ...
    const intl = useIntl();
    const {fetchBotSetup, updateBotSetup, fetchBotTemplates, isLoading, error} = useAPI();
    ...
    // Other maps of this owner whose bot could be reused as a starting point
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            setTemplates((await fetchBotTemplates()).filter((template) => template.id !== id));
        }
    ...
    async function applyTemplate(templateId) {
        const setup = await fetchBotSetup(templateId);
        if (setup) {
            setMessages(messagesFromTemplate(setup));
            setMaxAttempts(maxAttemptsFromTemplate(setup));
        }
    }
                        ...
                        {isQuestion(message.bot_step) &&
                            <button
                                ...
                                title={intl.formatMessage({
                                    id: "app.botSetup.removeQuestion",
    ...
    return (
        <>
            <div className="app">
                ...
                <div className="botSetup">
                    ...
                    <div className="botSetup__content">
                        ...
                        {templates.length > 0 &&
                            <div className="botSetup__template">
                                <SlSelect
                                    size="small"
                                    placeholder={intl.formatMessage({
                                        id: "app.botSetup.selectTemplatePlaceholder",
                                        defaultMessage: "Copy from another map",
                                    })}
                                    value={selectedTemplateId || ""}
                                    onSlChange={(event) => setSelectedTemplateId(event.target.value || null)}
                                >
                                    {templates.map((template) => (
                                        <SlOption key={template.id} value={template.id}>{template.name}</SlOption>
                                    ))}
                                </SlSelect>
                                <SlButton
                                    variant="default"
                                ...
                                >
                                    <FormattedMessage id="app.botSetup.useTemplate" defaultMessage="Use as template"/>
                                </SlButton>
                            </div>
                        }
            ...
            <ConfirmDialog
                ...
                setOpen={setConfirmTemplateOpen}
                onConfirm={applyTemplate}
                data={selectedTemplateId}
                title={{
                    id: "app.botSetup.templateConfirmTitle",
                    defaultMessage: "Replace this form with that map's bot?",
                }}
            >
                <FormattedMessage
                    id="app.botSetup.templateConfirmText"
                    defaultMessage="This replaces every message below with {name}'s configuration."
                    values={{name: templates.find((template) => template.id === selectedTemplateId)?.name}}
                />
            </ConfirmDialog>
        </>
    )
